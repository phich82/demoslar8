 ...
 *
 * 1. Custom Queue (CustomQueue)
 *    => Implement Illuminate\Contracts\Queue\Queue or extend Illuminate\Queue\Queue or both
 *    => Responsible for putting jobs on, taking jobs off and deleting jobs from the queue
 *       as well as gathering data such as the current queue size.
 * 2. Custom Connector (CustomQueueConnector)
 *    => Implement Illuminate\Queue\Connectors\ConnectorInterface
 *    => Build our own queue connector class.
 *    => responsible for connecting to your chosen queue service. Define yourself a class
 *       and implement Laravel's interface.
 * 3. Queue Service Provider (CustomQueueServiceProvider)
 *    => Create a service provider for registering this custom queue.
 *    => Add code lines bellow in `boot` method of ServiceProvider
 ...
 *       });
 * 4. Configuration
 *    => Open `config/queue.php` file and add the required configuration to the `connections` array:
 *       + The key should be the name of your driver (as supplied to addConnector in the queue service provider above)
 *       + The value should be an array of configuration options required by your drive.
 */
class RabbitMQQueue extends Queue implements QueueContract
{
    ...
    /**
     * @implement \Illuminate\Contracts\Queue\Queue
     *
     * Get current number of jobs on the queue
     *
     * @param  string $queue
     * @return int
    ...
    public function size($queue = null)
    {
        ...
        if (!$this->isExistQueue($queue)) {
            return 0;
        }

        // create a temporary channel for getting number of jobs on queue
     ...
     * @param  string $queue
     * @return void
     * @throws AMQPProtocolChannelException|\Illuminate\Queue\InvalidPayloadException
     */
     ...
     * @throws AMQPProtocolChannelException
     */
    public function isExistExchange(string $exchange)
    {
     ...
     * @throws AMQPProtocolChannelException
     */
    public function deleteExchange(string $name, bool $unused = false)
    {
        if (! $this->isExistExchange($name)) {
            return;
        }
     ...
     * @throws AMQPProtocolChannelException
     */
    public function isExistQueue(string $name = null)
    {
        try {
            // Create a temporary channel for checking existence of queue
            $channel = $this->connection->channel();
     ...
     * @return void
     */
    public function bindQueue(string $queue, string $exchange, string $routingKey = '')
    {
        if (in_array(
     ...
     * @throws AMQPProtocolChannelException
     */
    public function deleteQueue(string $name, bool $if_unused = false, bool $if_empty = false)
    {
     ...
     * @return void
     */
    public function purge(string $queue = null)
    {
        // create a temporary channel, so the main channel will not be closed on exception
        $channel = $this->connection->channel();
     ...
     * @return bool
     */
    protected function isQueueDeclared(string $name)
    {
    ...
    )
    {
        ...
        if ($exchange && ! $this->isExistExchange($exchange)) {
        ...
        if ($this->isExistQueue($destination)) {
            return;
        }
