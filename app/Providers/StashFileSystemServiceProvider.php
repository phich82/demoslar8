...
use League\Flysystem\Adapter\Local as StashAdapter;
...
class StashFileSystemServiceProvider extends ServiceProvider
{
    /**
     * Register services.
     *
    ...
    {
        // Create new storage driver (stash)
        Storage::extend('stash', function ($app, $config) {
            // Value of $config variable is as config('filesystem.stash)
            $adapter = new StashAdapter($config['root']);
