...
use League\Flysystem\Adapter\Local as PredisAdapter;
...
class PredisFileSystemServiceProvider extends ServiceProvider
{
    /**
     * Register services.
     *
    ...
    {
        // Create new storage driver (predis)
        Storage::extend('predis', function ($app, $config) {
            // Value of $config variable is as config('filesystem.stash)
            $adapter = new PredisAdapter($config['root']);
